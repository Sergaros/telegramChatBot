...
const parseDate = (action, parameters) => {
	...
	if(typeof(parameters.date) === 'string') {
		return;
	} else if(parameters.date.date_time) {
...
const prepareQuery = (chat_id, parameters) => {
	...
	if(parameters.date) {
		query.date = {
			$gte: moment(parameters.date).startOf('day').toDate(),
			$lt: moment(parameters.date).endOf('day').toDate()
		};
	} else if(parameters.startDate && parameters.endDate) {
		query.date = {
			$gte: moment(parameters.startDate).startOf('day').toDate(),
			$lt: moment(parameters.endDate).endOf('day').toDate()
		};
	}
...
const sendMessage = async (chat_id, text, quickReplies) => {
	await axios.get(`https://api.telegram.org/bot${config.telegram.token}/sendMessage`, {
		params: {
			chat_id,
			text,
